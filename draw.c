...
void add_sphere( struct matrix * edges, 
  ...
  for (int i = 0; i < t->lastcol; i++) {
    add_edge(edges,
	     t->m[0][i], t->m[1][i], t->m[2][i],
	     t->m[0][i], t->m[1][i], t->m[2][i] + 1);
  }
...
struct matrix * generate_sphere(double cx, double cy, double cz,
  ...
  struct matrix *t = new_matrix(4, step);
...
struct matrix * generate_torus( double cx, double cy, double cz,
  ...
  for (int i = 0; i < step; i++) { 
    for (int x = 0; x < step; x++) {
      ...
      t2 = 2 * M_PI * x/step;
      add_point(t,
		(r1 * cos(t2) + r2) * cos(t1) + cx,
		r1 * sin(t2) + cy,
		(r1 * cos(t2) + r2) * -1 * sin(t1) + cz);
    }
  }
  return t;
}
